...
import type { Event, EventStatus, EventTheme } from '../types';

const THEMES: ReadonlyArray<{ id: EventTheme; label: string; emoji: string }> = [
...
type TransitionAction = 'publish' | 'close' | 'reopen' | 'archive';
...
export default function EventEdit() {
  ...
  const { register, handleSubmit, watch, setValue, reset, formState: { errors, isDirty } } = useForm<EditFormData>({
    resolver: zodResolver(schema) as Resolver<EditFormData>,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (vals: EditFormData) =>
      eventsApi.update(id!, {
        ...vals,
        max_guests:
          typeof vals.max_guests === 'number'
            ? vals.max_guests
            : vals.max_guests
              ? parseInt(String(vals.max_guests), 10)
              : null,
  ...
  const transitionMutation = useMutation({
    mutationFn: (action: TransitionAction) => eventsApi[action](id!),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['event', id] });
      qc.invalidateQueries({ queryKey: ['events', 'mine'] });
    },
  ...
  async function handleCoverUpload(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      qc.setQueryData(['event', id], (old: Event | undefined) =>
        old ? { ...old, cover_image_url: res.data.cover_image_url } : old,
      );
    } catch {
  ...
  const onSubmit: SubmitHandler<EditFormData> = (v) => saveMutation.mutate(v);
