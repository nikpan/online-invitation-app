...
const STEPS = ['Basics', 'Customize', 'Settings', 'Review'];

export default function EventBuilder() {
  const navigate = useNavigate();
  const [step, setStep] = useState(0);
  const [data, setData] = useState({ theme: 'confetti', is_public: true });
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState('');

  const schemas = [step1Schema, step2Schema, step3Schema, null];
  ...
  async function onSubmit(values) {
    ...
    try {
      const res = await eventsApi.create({
        ...
        theme: final.theme,
        max_guests: final.max_guests ? parseInt(final.max_guests) : null,
