...
export default function GuestList() {
  ...
  const [addForm, setAddForm] = useState({ name: '', email: '', rsvp_status: 'attending', adult_count: 1, kid_count: 0 });
  ...
  const { data: guestData, isLoading } = useQuery({
    queryKey: ['guests', id, filter],
    queryFn: () => guestsApi.list(id, { status: filter || undefined, limit: 200 }).then((r) => r.data),
  });

  const removeMutation = useMutation({
    mutationFn: (guestId) => guestsApi.remove(id, guestId),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['guests', id] }),
  });

  const addMutation = useMutation({
    mutationFn: (data) => guestsApi.add(id, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['guests', id] });
      setShowAddForm(false);
      setAddForm({ name: '', email: '', rsvp_status: 'attending', adult_count: 1, kid_count: 0 });
      setAddError('');
    },
    onError: (err) => setAddError(err.response?.data?.error || 'Failed to add guest'),
  });
  ...
  const totalAttending = guests.filter((g) => g.rsvp_status === 'attending').reduce((sum, g) => sum + g.adult_count + g.kid_count, 0);
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setAddForm((p) => ({ ...p, rsvp_status: e.target.value }))}
                >
              ...
              <div className="flex gap-3">
                <div className="flex-1">
                  ...
                  <input
                    ...
                    onChange={(e) => setAddForm((p) => ({ ...p, adult_count: parseInt(e.target.value) }))}
                  />
                </div>
                <div className="flex-1">
                  ...
                  <input
                    ...
                    onChange={(e) => setAddForm((p) => ({ ...p, kid_count: parseInt(e.target.value) }))}
                  />
                </div>
              </div>
            </div>
